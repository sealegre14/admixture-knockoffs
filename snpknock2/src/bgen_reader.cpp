...
void BgenReader::summarise() {
  ...
  try {
    ...
    std::cout << "Exception occurred whbile trying to open BGEN file " << filename << std::endl;
    exit(1);
  }
...
std::pair<bool, bool> BgenReader::prob_to_hap(const vector<double>& probs) const {
  ...
  // SA modifying to print what K is if K != 4 
  if (K != 4) {  
    std::cerr << "DEBUG: K = " << K << " (expected 4) - probs.size() = " << probs.size() << std::endl;  
    // Optionally print first few probability values  
    std::cerr << "First few probabilities: ";  
    for (int i = 0; i < std::min(5, (int)probs.size()); i++) {  
      std::cerr << probs[i] << " ";  
    }  
    std::cerr << std::endl;  
  }  
