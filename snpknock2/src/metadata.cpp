...
void Legend::push_back(vector<string> row_) {
  ...
  if(row_.size()==4) {
    ...
    // SA edit start 
    // legend format, without cM  
    // Extract chromosome from SNP ID like "22:17103078:C:G"  
    string snp_id = row_[0];  
    size_t colon_pos = snp_id.find(':');  
    if(colon_pos != string::npos) {  
      chr.push_back(snp_id.substr(0, colon_pos));  
    } else {  
      chr.push_back("?");
    }
    // SA edit end - need to return to original by just only havein the chr push back ? line 
    ID.push_back(row_[0]);
