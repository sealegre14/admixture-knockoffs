...
# Load genetic map
# SA commenting this out and just changing column names to what .gmap calls them 
...
# debug 
## After loading data  
cat("Variants loaded:", nrow(Variants), "rows\n")  
cat("Map loaded:", nrow(Map), "rows\n")  
  
## After nearest neighbor join  
cat("After join:", nrow(Map), "rows\n")  
cat("Unique Map positions:", length(unique(Map$Map)), "\n")  
  
## Check if Map column exists  
if(!"Map" %in% colnames(Map)) {  
  cat("ERROR: Map column missing!\n")  
  print(head(Map))  
}
# end of debug
...
#SA TRYING DIFF BLOCK TO GO TO NEIGHREST NEIGHBORS 
## Cross-reference genetic map and list of variants using nearest positions  
library(data.table)  
  
# Convert to data.table for fast rolling joins  
variants_dt <- as.data.table(Variants)  
map_dt <- as.data.table(Map)  
  
# Roll join to find nearest genetic map position for each variant  
variants_dt <- map_dt[variants_dt, on = c("CHR", "BP"), roll = "nearest"]  
Map <- as_tibble(variants_dt)
